...
* **Do not treat Greek letters as Latin lookalikes:** Α (Greek alpha) and A (Latin A) are different [characters](../../programming-terms/text-in-software/character.md) with different [code points](../../programming-terms/text-in-software/code-point.md), even when they share a shape. Matching, sorting, or de-duplicating on the shape breaks, and mixing the two invites spoofed text, so validate and compare by script, not by appearance.
