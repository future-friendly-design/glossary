...
The visible headstroke that runs along it in [Devanagari](devanagari.md) is the [shirorekha](shirorekha.md); the hanging baseline is the invisible alignment line, not the stroke itself. In नमस्ते (namaste), for instance, every letter hangs from the unbroken line across the top of the word, and that top line is the hanging baseline.
...
| --------------------------------------- | ---------------------- | ---------------------------------------------------------------------------------- | ---------------- |
| [Devanagari](devanagari.md)             | Hindi, Marathi, Nepali | Most symbols hang from one continuous top line.<sup>1</sup>                        | नमस्ते (namaste) |
| [Bengali-Assamese](bengali-assamese.md) | Bengali, Assamese      | Letters join along a top bar, and the hanging baseline is based on it.<sup>2</sup> | বাংলা (bangla)   |
| [Tibetan](tibetan-script.md)            | Tibetan, Dzongkha      | Glyphs hang from a strong, though not absolute, top edge.<sup>3</sup>              | བོད་ཡིག (böyig)  |

Whether a script hangs is decided per script, not by family: Gujarati, Marathi, and Punjabi hang as well, but other [Brahmic scripts](brahmic-scripts.md) such as [Tamil](tamil-script.md), Telugu, and Malayalam sit on a bottom [baseline](../../design-terms/typography/baseline.md) like [Latin](latin-script.md).<sup>4</sup>
