...
When a written language is read, the smallest symbol, or combination of symbols, that distinguishes one meaning from another is a [grapheme](../../programming-terms/text-in-software/grapheme.md). For example, `e` is one symbol and one grapheme; `é` is two symbols, a letter plus an accent mark, but still one grapheme.
...
In computer programming, there is a universal coding standard called [Unicode](../../programming-terms/text-in-software/unicode.md). What this glossary calls a symbol, Unicode refers to as a [character](../../programming-terms/text-in-software/character.md). Each character is given a unique ID number called a [code point](../../programming-terms/text-in-software/code-point.md). They reserve the term `symbol` for math symbols like `+`, currency symbols like `$`, modifier symbols, and other symbols such as `©` and emoji `🔠`.<sup>1</sup>
