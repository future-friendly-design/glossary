...
level: intermediate
depth: core
summary: A code unit is the smallest unit of storage an encoding uses to represent text.
related: [code-point, utf-8, utf-16, surrogate-pair]
status: voice-passed
version_added: 0.1
updated: 2026-08-04
...
Its size depends on the encoding: [UTF-8](utf-8.md) uses 8-bit code units, [UTF-16](utf-16.md) uses 16-bit ones.<sup>2</sup>

### Why it matters in design systems

A [code point](code-point.md) and a code unit answer different questions. The code point is the character's identity, the number Unicode assigned it. The code unit is a chunk of computer storage,<sup>3</sup> and how many of them a character needs depends entirely on which encoding is doing the storing. The same character is one code point always, but one, two, three, or four code units depending on the encoding.
...
So when you write a character limit into a spec, say which unit you mean. Code units are a storage detail that should almost never surface to a person. What a reader counts is closer to a [grapheme cluster](grapheme-cluster.md), and that is the unit a user-facing limit should use.

***

### Related terms and mentions

[Character](character.md) · [Character encoding](character-encoding.md) · [Code point](code-point.md) · [Grapheme cluster](grapheme-cluster.md) · [Language](../../language-terms/linguistics/language.md) · [Script](../../language-terms/writing-systems-and-scripts/script.md) · [Surrogate pair](surrogate-pair.md) · [Unicode](unicode.md) · [UTF-8](utf-8.md) · [UTF-16](utf-16.md) · [Text in software](../text-in-software/)
