...
depth: deep
summary: >-
  Segmentation is the process of dividing text into meaningful units such as
...
So what counts as a character, word, or sentence boundary differs by [writing system](../../language-terms/writing-systems-and-scripts/writing-system.md), and segmentation cannot rely on a simple rule like splitting on spaces. [Unicode](unicode.md) Standard Annex #29 defines default algorithms for [grapheme cluster](grapheme-cluster.md) boundaries (the unit a reader counts as one character), word boundaries, and sentence boundaries, but it is explicit that scripts without spaces between words need dictionary-based tailoring rather than the default.<sup>1</sup>
