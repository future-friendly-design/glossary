...
## Why it matters
In an abjad the consonant symbols are primary and vowel sounds are typically omitted, or only optionally shown with [[diacritic]] marks. Arabic and Hebrew, the two abjads most relevant to interface work, are both written right to left, so supporting them brings in [[bidirectional-text|bidirectional text]] handling on top of the script's own behavior. The fact that short vowels are normally unwritten also affects how search, matching, and text input work for these scripts.
