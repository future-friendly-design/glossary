...
| [Joining](../language-terms/writing-systems-and-scripts/joining.md) | Letters connect cursively, most taking an isolated, initial, medial, or final form depending on their position in the word<sup>4</sup> | The join is structural, not decorative: you cannot add [letter spacing](letter-spacing.md) that pulls the joined letters apart |
| [Ligature](ligature.md) (lam-alef) | The lam-alef combination forms an obligatory ligature where the font's style supports it<sup>5</sup> | Confirm the font ships it; it is expected, not optional |
| [Text direction](../language-terms/writing-systems-and-scripts/text-direction.md) | Right to left<sup>6</sup> | A Latin word or number that shares a line is [bidirectional](../language-terms/writing-systems-and-scripts/bidirectional-text.md) and has to be reordered by the Unicode bidirectional algorithm to read correctly<sup>7</sup> |
