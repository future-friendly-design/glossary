...
further_reading:
  - title: "Armenian alphabet (Wikipedia)"
    url: https://en.wikipedia.org/wiki/Armenian_alphabet
    type: authority
license: CC-BY-4.0
---
...
## Definition
The Armenian script is an alphabet used to write the Armenian language, with its own distinct letters for consonants and vowels.
...
## Example
Armenian has its own alphabet, unrelated in letterforms to Latin or Cyrillic, so a font with full Latin coverage may have no Armenian glyphs at all.

## Related terms
[Alphabet](alphabet.md) · [Script](script.md) · [Greek script](greek-script.md)

## Further reading
- Foundations: [Armenian alphabet (Wikipedia)](https://en.wikipedia.org/wiki/Armenian_alphabet)

<!-- NEEDS EXPERT REVIEW: source is Wikipedia (interim). Upgrade to an authoritative primary (Unicode Standard Section 7.6 Armenian, ScriptSource: Armn) vetted by the SILCON cohort. The single-language characterization and any historical-origin or date claims need cohort confirmation; no letter counts are asserted. -->
