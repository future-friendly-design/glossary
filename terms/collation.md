...
## In practice
- **Use a locale-aware comparator:** in JavaScript, `Intl.Collator(locale).compare` (or `localeCompare`) sorts correctly per locale; server-side, lean on [[icu]] rather than a naive string compare. See [Intl.Collator (MDN)](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/Collator).
