...
Where you cannot be creative is the script rules. Text runs left to right, the direction Western tools already assume,<sup>4</sup> and Cyrillic is bicameral, with two cases.<sup>5</sup> The constraint that catches teams is coverage across languages: Cyrillic serves dozens of them, so "supports Cyrillic" is not a single guarantee. Languages use overlapping but different letter sets, and some letters are shaped differently by language, so several italic letters look different in Serbian and Bulgarian than in Russian.<sup>6</sup> A font has to carry the letters AND the language's forms, which the [OpenType](opentype.md) localized-forms feature selects from the text's declared language. Pick the wrong forms and the text reads as foreign to a native reader, even though every letter is present.
...
3. The details of the history of the development of the Cyrillic script, and of the relationship between early writing systems for Slavic languages, have been lost - The Unicode Standard, Version 16.0, Chapter 7: Europe-I [https://www.unicode.org/versions/Unicode16.0.0/core-spec/chapter-7/](https://www.unicode.org/versions/Unicode16.0.0/core-spec/chapter-7/)
4. The Cyrillic script is written in linear sequence from left to right - The Unicode Standard, Version 16.0, Chapter 7: Europe-I [https://www.unicode.org/versions/Unicode16.0.0/core-spec/chapter-7/](https://www.unicode.org/versions/Unicode16.0.0/core-spec/chapter-7/)
...
6. The shapes of the italic forms of Cyrillic letters can vary by language - Cyrillic (r12a script notes) [https://r12a.github.io/scripts/cyrl/ru.html](https://r12a.github.io/scripts/cyrl/ru.html)
