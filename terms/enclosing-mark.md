...
## Why it matters
This completes the third and last member of Unicode's "Mark" category, alongside the nonspacing [combining mark](combining-mark.md) and the [spacing combining mark](spacing-combining-mark.md). An enclosing mark, General Category Me, is a kind of nonspacing mark that wraps around its base rather than sitting above or beside it, producing things like a digit inside a circle. For software the same rule applies as for other combining marks: the base and the enclosing mark together are a single [grapheme cluster](grapheme-cluster.md), so cursor movement, selection, and counting should treat them as one unit, not two.
