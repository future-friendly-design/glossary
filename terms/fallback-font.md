...
## Why it matters
In CSS, `font-family` is a priority list (a "font stack"), and the browser picks the first available font that can render each character, working one character at a time. Ending the stack with a generic family like `sans-serif` guarantees there is always an acceptable substitute. Fallbacks do double duty: they cover the case where the preferred font fails to load, and the case where it loads but lacks a glyph for a particular character, which ties directly to [[font-coverage]].
...
- **Coverage:** fallbacks are the safety net for [[font-coverage]] gaps, since a character the primary font lacks is drawn by the next font in the stack that has it. Order the stack so that substitute is acceptable for every script you support.
...
## Related terms
[[font-coverage]] · [[font-subsetting]] · [[font]]
