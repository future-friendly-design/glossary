...
| Marks | vowel points (niqqud), the dagesh, and cantillation marks, positioned mostly below and within the consonants as combining [marks](../language-terms/writing-systems-and-scripts/mark.md) |
...
| Niqqud (vowel points) | optional vowel [marks](../language-terms/writing-systems-and-scripts/mark.md), positioned mostly below the consonants and normally omitted outside liturgical and learning texts |
...
Where you cannot be creative is the script rules. Text runs right to left, so a number or a Latin word that shares a line is [bidirectional](../language-terms/writing-systems-and-scripts/bidirectional-text.md) and has to be reordered to read correctly.<sup>3</sup> Unlike the [Arabic script](arabic-script.md), Hebrew letters do not join, so a letter keeps its shape wherever it sits,<sup>4</sup> but five of them take a distinct form at the end of a word,<sup>5</sup> and when vowel points are shown they are combining marks that have to be positioned on the consonants.<sup>6</sup> This layout, the right-to-left order, the final forms, and any point placement, is carried by the font's [OpenType](opentype.md) rules and applied at render time by the platform's [text shaping](../programming-terms/text-for-digital-products-and-the-web/text-shaping.md), the step that turns stored characters into positioned glyphs. It is not a set of styling choices: get the direction or the final forms wrong and the text reads incorrectly, not just unstyled.
...
* **Check the orthography, not just the script, before reusing symbols across languages:** Hebrew, Yiddish, and Ladino all use the Hebrew script and still differ in how they use it, for example Yiddish writes its vowels out with letters where Hebrew leaves them to points. Do not hardcode one language's choices; pull them from [locale](locale.md) data. Unicode's [CLDR](cldr.md) publishes per-language conventions as machine-readable data.
...
4. A handwritten form of Hebrew is known as cursive, but its rounded letters are generally unconnected, so the letters do not join - The Unicode Standard, Version 16.0, Chapter 9: Middle East-I [https://www.unicode.org/versions/Unicode16.0.0/core-spec/chapter-9/](https://www.unicode.org/versions/Unicode16.0.0/core-spec/chapter-9/)
5. Variant forms of five Hebrew letters are encoded as separate characters and are generally used in place of the nominal letterforms at the end of words - The Unicode Standard, Version 16.0, Chapter 9: Middle East-I [https://www.unicode.org/versions/Unicode16.0.0/core-spec/chapter-9/](https://www.unicode.org/versions/Unicode16.0.0/core-spec/chapter-9/)
