...
| [Text direction](../language-terms/writing-systems-and-scripts/text-direction.md) | [Vertical](vertical-text.md), top to bottom, in columns that run left to right<sup>9</sup> | You cannot reuse horizontal-layout assumptions, so verify your layout stack can set vertical, left-to-right columns, not just horizontal lines |
| [Text shaping](../programming-terms/text-for-digital-products-and-the-web/text-shaping.md) | Shaping happens at render time: the [shaping engine](shaping-engine.md) reads the font's OpenType features (see OpenType above) and selects each letter's correct positional and variant form from its context<sup>10</sup> | A correct font is necessary but not sufficient: a tool or pipeline that places glyphs without shaping renders Mongolian wrong even with a fully-featured font, and vertical layout compounds it. Test in the actual design tools your team uses, not just the browser |
...
* **Confirm vertical layout before you commit a typeface or a tool:** verify the font ships the cursive joining and positional forms AND that your layout stack can set vertical, left-to-right columns, not just horizontal lines. See [font coverage](font-coverage.md).
