...
## In practice
- **Never derive visual position from logical order:** for cursor movement, selection, and truncation in these scripts, rely on the platform's text APIs (which know the shaped result), not raw string indexing. This is the same discipline [[grapheme-cluster]] segmentation calls for, applied to order rather than counting.
- **Distinguish it from bidi:** [[bidirectional-text]] reordering and intra-syllable reordering are different mechanisms; a layout can need both, so do not assume handling one covers the other.
...
## Related terms
[[matra]] · [[reph]] · [[stacking-script]] · [[complex-text-layout]] · [[bidirectional-text]]
