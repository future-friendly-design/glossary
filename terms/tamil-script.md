...
| --- | --- | --- |
| [Baseline](../design-terms/typography/baseline.md) | Symbols sit on a bottom (alphabetic) baseline, not hanging from a headstroke ([shirorekha](../language-terms/writing-systems-and-scripts/shirorekha.md))<sup>3</sup> | Tamil sits on a bottom baseline, so a line that mixes it with Latin shares one baseline |
...
| [Conjuncts](../language-terms/writing-systems-and-scripts/conjunct.md) | Tamil forms only one consonant conjunct, writing consonant clusters with a visible pulli instead<sup>5</sup> | Consonant clusters stay as separate symbols marked with the pulli rather than fusing, so do not expect heavy conjunct behaviour |
| [Font](font.md) (open source) | [Noto Sans Tamil](https://fonts.google.com/noto/specimen/Noto+Sans+Tamil) is a free, open-source font from Google's [Noto](noto-fonts.md) project, covering the script's characters and the OpenType features they need (see OpenType and Text shaping) | Treat the typeface as a foundational choice, and confirm it covers every language you support that uses the script (see [font coverage](font-coverage.md)), not just one |
...
* **Cover the script before you commit a typeface:** confirm the font ships the Tamil glyphs AND the [shaping rules](../programming-terms/text-for-digital-products-and-the-web/text-shaping.md) that position them, not just the base consonants. See [font coverage](font-coverage.md).
