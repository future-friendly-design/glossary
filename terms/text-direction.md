...
So for a design system, no script rule reaches further than direction, and setting the base direction is only the first step. Switching it ripples through every layer of the system. At the text layer, alignment and the starting edge of each line flip, so a right-to-left paragraph aligns to the right and begins at the right edge. In components, the internal layout mirrors: a button's icon moves to the leading edge, and directional icons such as back and forward arrows point the other way.<sup>4</sup> At the page level, navigation and the overall reading flow mirror to match. And in code, none of this holds unless direction is set at the document level and the layout is built with logical CSS properties, such as `margin-inline` and `text-align: start`, that follow the text instead of being pinned to a physical edge; values hardcoded to `left` or `right` break when the direction changes. That groundwork is what makes [complex text layout](complex-text-layout.md) possible.

Direction is just one of a script's [script rules](../language-terms/writing-systems-and-scripts/script-rules.md), and the same cross-level cascade applies to each of them, so check the others a script you support might have, such as [joining](../language-terms/writing-systems-and-scripts/joining.md) and [stacking](../language-terms/writing-systems-and-scripts/stacking-script.md). And on top of the script rules, each language that uses the script has its own [orthography](../language-terms/writing-systems-and-scripts/orthography.md), the spelling and punctuation conventions that can add further requirements.
