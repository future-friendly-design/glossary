...
## Why it matters
When text contains a character that the current font and its [[fallback-font]]s cannot draw, the renderer shows a placeholder, often a plain rectangle that designers nicknamed "tofu". The key thing it tells you: this is a missing [[glyph]], not a broken character or a bad encoding, so the fix is font coverage, not re-saving the file. Google's Noto font family is even named "No Tofu" because it aims to cover as many scripts as possible to eliminate these boxes.

## Example
A page using a Latin-only font may show tofu boxes where Tibetan or emoji should appear.

## Related terms
[[glyph]] · [[font]] · [[font-coverage]]
