...
## Why it matters
This is the design itself, the artwork, separate from any one file that delivers it. A typeface is the whole family (Helvetica, a widely used sans-serif), and a [[font]] is one specific instance inside it (Helvetica Bold at 12pt). In everyday speech people swap the two words freely, and that is fine in conversation. It starts to bite the moment you are specifying type for a real product, because "use Helvetica" does not tell anyone which weight or style, and a design system needs that precision.
...
## Common mistake
Treating a typeface as a single thing you can just "apply," when what you actually ship is a set of fonts: regular, bold, italic, and so on. If you pick a typeface that only ships a regular weight and then ask for bold anyway, the tool or browser may fake it (see [[faux-bold]]), which distorts the letterforms. Choosing a typeface means checking that the family actually includes the weights and styles your design depends on.

## In practice
- **Tokens:** the typeface (the family name) is usually one token, like a `font-family` value, while the weights and styles it contains are separate decisions ([[font-weight]], italics). Keeping the family as its own token lets you swap the whole typeface in one place without rewriting every weight reference.
...
## Related terms
[[font]] · [[font-weight]] · [[variable-font]]
